    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (attacker instanceof Player player) {
            ...
            if (data.hasStartedCultivation()) {
                ...
                // 应用功法伤害加成
                if (technique.isPassiveSkill() && (technique.name().startsWith("BLADE_"))) {
                    float damageMultiplier = (float) technique.getDamageMultiplier();
                    target.hurt(player.level().damageSources().playerAttack(player), 3.0F * (damageMultiplier - 1.0F));
                }
                
                // 应用技能伤害加成
                float skillMultiplier = SkillManager.getDamageMultiplier(player);
                if (skillMultiplier > 1.0F) {
                    target.hurt(player.level().damageSources().playerAttack(player), 3.0F * (skillMultiplier - 1.0F));
                }
