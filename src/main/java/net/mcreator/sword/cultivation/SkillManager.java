...
public class SkillManager {
    ...
    private static void castEarthSpike(Player player, Level level) {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            DustParticleOptions dustOptions = new DustParticleOptions(new Vector3f(0.8F, 0.8F, 0.8F), 1.0F);
            level.addParticle(dustOptions, x, hitPos.y, z, 0, 1, 0);
        }
    ...
    private static void castElementalBurst(Player player, Level level) {
        ...
        for (Entity entity : level.getEntitiesOfClass(LivingEntity.class, area)) {
            if (entity != player) {
                entity.hurt(level.damageSources().playerAttack(player), 15.0F);
    ...
    private static void castSoulResonance(Player player, Level level) {
        ...
        for (int i = 0; i < 40; i++) {
            ...
            level.addParticle(ParticleTypes.EFFECT, x, player.getY() + 1, z, 0, 0.1, 0);
        }
